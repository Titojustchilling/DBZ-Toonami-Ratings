...
while quit == 0: 
    ...
    if whatDoYouWant == 'graph households': 
        df = pd.read_csv('Dragon Ball Z Full Ratings - Sheet1 (1).csv')
        ...
        plt.show()
            ...
            if saga.lower() == 'frieza' or saga.lower() == 'frieza saga':
                df = pd.read_csv('Frieza Saga - Sheet1.csv', usecols=['Sega', 'Episode Titles', 'Household', 'Rank', 'English air date',  'Viewership']) 
                ...
                df = pd.read_csv('Garlic Jr. Trunks and Androids Saga - Sheet1.csv', usecols=['Sega ', 'Episode Titles', 'HH', 'Rank ', 'English air date', 'Viewership']) 
                rowIndex = userInput - 1
                episodeTitle = df.loc[rowIndex, 'Episode Titles'].strip()
                ...
                df = pd.read_csv('Imperfect Cell and Perfect Cell Sagas - Sheet1.csv', usecols=['Sega', 'Episode Titles ', 'English air date ', 'HH', 'Rank', 'Viewership']) 
                rowIndex = userInput - 1
                episodeTitle = df.loc[rowIndex, 'Episode Titles '].strip()
                rating = df.loc[rowIndex, 'HH'] 
                rank = df.loc[rowIndex, 'Rank']
                ...
                df = pd.read_csv('Cells Games - Sheet1.csv', usecols=['Sega', 'Episode Titles', 'Episode air date', 'HH', 'Rank', 'Viewership'])
                rowIndex = userInput - 1
                episodeTitle = df.loc[rowIndex, 'Episode Titles'].strip()
                rating = df.loc[rowIndex, 'HH'] 
                rank = df.loc[rowIndex, 'Rank']
                ...
                df = pd.read_csv('Other World, Great Saiyaman and World Tournament  - Sheet1 (1).csv', usecols=['Sega', 'Episode Titles', 'Episode air date', 'HH', 'Rank', 'Viewership'])
                rowIndex = userInput - 1
                episodeTitle = df.loc[rowIndex, 'Episode Titles'].strip()
                rating = df.loc[rowIndex, 'HH'] 
                rank = df.loc[rowIndex, 'Rank']
                ...
                df = pd.read_csv('Babidi and Majin Buu - Sheet1.csv', usecols=['Sega', 'Episode Titles', 'Episode air date', 'HH', 'Rank', 'Viewership '])
                rowIndex = userInput - 1
                episodeTitle = df.loc[rowIndex, 'Episode Titles'].strip()
                rating = df.loc[rowIndex, 'HH'] 
                rank = df.loc[rowIndex, 'Rank']
                ...
                df = pd.read_csv('Fusion, Kid Buu and Peaceful World Sagas - Sheet1.csv', usecols=['Saga', 'Episode Titles', 'Episode air date', 'HH', 'Rank', 'Viewership']) 
                rowIndex = userInput - 1
                episodeTitle = df.loc[rowIndex, 'Episode Titles'].strip()
                rating = df.loc[rowIndex, 'HH'] 
                rank = df.loc[rowIndex, 'Rank']
